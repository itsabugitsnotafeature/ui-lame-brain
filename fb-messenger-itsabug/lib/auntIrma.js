...
class Irma {
  ...
  /*
		Constructor : Returns new object initialized with master DATE.
    @date          : Default to kryptonite Date of last occuence timestamp.
    @month         : Default to kryptonite month of last occuence timestamp. 
    @year          : Default to kryptonite year of last occuence timestamp.
    @controlCyclePeriod    : Default to kryptonite controlCyclePeriod.
    @visitDuration : Default to kryptonite's total length of duration.
	*/ 
  constructor( sufferer = 'kryptonite', date = T_DATE, month = T_MONTH, year = T_YEAR, cycle = T_CYCLE, visitDuration = T_DURATION){
  ...
  calculateFutureVisits() {
    ...
    thisUser.yearlyPredictions.sufferer = this.sufferer;      
